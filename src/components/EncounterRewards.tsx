...
export default function EncounterRewards({ tamanhoGrupo }: { tamanhoGrupo: number }) {
  ...
  const texto = [`Recompensa do encontro: ${loot.valorTotal} PO de valor total`, `${loot.moedas} PO em moedas`, ...loot.itens.map((i) => `• ${i.name} (${i.price} PO): ${i.description}`)].join("\n");
  return <details className="mt-6 rounded-xl border border-parchment-300 p-4 dark:border-parchment-700">
    ...
    <ul className="my-3 space-y-2 text-sm">{loot.itens.map((i, indice) => <li key={`${i.id}-${indice}`}><strong>{i.name}</strong> · {i.price} PO<p className="text-xs text-parchment-600 dark:text-parchment-400">{i.description}</p></li>)}</ul>
